<!doctype html>
<html lang="en">

<head>
  <!-- Required meta tags -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <!-- Bootstrap CSS -->
  <link href="assets/scss/bootstrap.min.css" rel="stylesheet">
  <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css"
    integrity="sha384-wvfXpqpZZVQGK6TAh5PVlGOfQNHSoD2xbE+QkPxCAFlNEevoEH3Sl0sibVcOQVnN" crossorigin="anonymous">
  <link href="assets/scss/main.css" rel="stylesheet">
  <title>GSISd | Our SD-WAN Solution</title>
  <meta name="description"
    content="Get GSISD SD-WAN solution for your business that gives security with flexibility." />
</head>

<body>
  <?php include 'header.php';?>

  <div class="section">
    <div class="row">
      <p>Deploy flexible and reliable modern applications and secured hybrid enterprise
        WAN with</p>
      <h1>GSISD SD-WAN</h1>
    </div>
    <div class="video-container">
      <video autoplay loop muted poster="assets/images/GSISD-SDWAN-Solution.jpg">
        <source src="assets\images\video.mp4" type="video/mp4">
      </video>
    </div>
  </div>
  <main class="">
    <section class="sdwan">
      <div class="container">
        <div class="row spacer border-bottom">
          <h2 class="main-title mb-4 text-center">Time to revamp your traditional, MPLS-based WAN!</h2>
          <p class="lead text-center px-5">In this fast pace demanding world, relieve your shoulders from day to day
            challenges
            of managing network-related issues due to high bandwidth consuming apps. Take benefits of our economic,
            reliable and flexible SD-WAN deployed on a Network as a Service model.</p>
        </div>
        <div class="row spacer">
          <h2 class="main-title mb-4 text-center">What is SD-WAN?</h2>
          <div class="col-md-6 my-auto">
            <img src="assets\images\GSISD SDWAN.png" alt="GSISD SDWAN" class="img-fluid">
          </div>
          <div class="col-md-6 lead mt-5 px-5">
            <p>
              Software-Defined Wide Area Network or SD-WAN is a next-gen connectivity solution for your business.</p>
            <p>Traditional WANs routed all traffic to a central HQ which increase the loss in productivity if business
              use cloud for services like Amazon Web Services, Salesforce and other SaaS services as they are better if
              they are send straight to the internet.</p>
            <p>Hence, unlike traditional WANs SD-WAN dependence on multiple ISPs with a single software solution, with
              One Point of Contact. SD-WAN is hosted completely on the cloud so it minimizes the need for hardware
              infrastructure</p>
          </div>
        </div>
      </div>
    </section>
    <!--gateways-->
    <section class="gateways spacer border-top" id="sdsolutions">
      <div class="container">
        <h2 class="main-title text-center">GSISD SD-WAN Solutions</h2>
        <div class="row mt-5">
          <div class="col-md-4 col-lg-3 mb-2">
            <div class="card text-white" style="border-color: #c8c6a7;">
              <h5 class="fw-bold card-title text-white h4 card-header"
                style="background-color: #c8c6a7;border-color: #c8c6a7;">GSISd Manage</h5>
              <div class="card-body text-center">
                <ul class="gateways__list">
                  <li>Managed Service</li>
                  <li>Predictive Analytics</li>
                  <li>Cloud Orchestration</li>
                  <li>Worldwide SLAs – 5-9s, Day-1 SLA</li>
                  <li>Flexible Subscriptions</li>
                  <li>ANAP – Secure Managed CPE</li>
                </ul>
              </div>
            </div>
          </div>
          <div class="col-md-4 col-lg-3 mb-2">
            <div class="card  text-white" style="border-color: #6e7c7c;">
              <h5 class="fw-bold card-title text-white h4 card-header"
                style="background-color: #6e7c7c;border-color: #6e7c7c;">GSISd Connect</h5>
              <div class="card-body text-center">
                <ul class="gateways__list">
                  <li>S2S InternetVPN (HybridWAN)</li>
                  <li>Inter-Region Usage</li>
                  <li>GSISD Core (PoPs)</li>
                  <li>Bursting</li>
                  <li>Last Mile Services</li>
                  <li>High Availability (CPE/PoP)</li>
                </ul>
              </div>
            </div>
          </div>
          <div class="col-md-4 col-lg-3 mb-2">
            <div class="card text-white" style="border-color: #435560;">
              <h5 class="fw-bold card-title text-white h4 card-header"
                style="background-color: #435560;border-color: #435560;">GSISd Optimize</h5>
              <div class="card-body text-center">
                <ul class="gateways__list">
                  <li>TurboApp - Application Acceleration</li>
                  <li>TurboNet - Network Acceleration</li>
                </ul>
              </div>
            </div>
          </div>
          <div class="col-md-4 col-lg-3 mb-2">
            <div class="card  text-white" style="border-color: #ffc288;">
              <h5 class="fw-bold card-title text-white h4 card-header"
                style="background-color: #ffc288;border-color: #ffc288;">GSISd Cloud</h5>
              <div class="card-body text-center">

                <ul class="gateways__list">
                  <li>SaaS Acceleration</li>
                  <li>Azure VWAN (HybridWAN)</li>
                  <li>IaaS Direct Connect (AWS, Azure, etc.)</li>
                </ul>
              </div>
            </div>
          </div>
        </div>
        <div id="sdsolutions2">
          <div class="row">
            <div class="col-md-4 col-lg-3 mb-2">
              <div class="card text-white" style="border-color: #00adb5;">
                <h5 class="fw-bold card-title text-white h4 card-header"
                  style="background-color: #00adb5;border-color: #00adb5;">GSISd Secure</h5>
                <div class="card-body text-center">
                  <ul class="gateways__list">
                    <li>Virtual Firewall Hosting</li>
                    <li>Secure Remote Access</li>
                    <li>Edge Essentials (Firewall, Micro-Segmentation)</li>
                    <li>Cloud Security Connector</li>
                  </ul>
                </div>
              </div>
            </div>
            <div class="col-md-4 col-lg-3 mb-2">
              <div class="card text-white" style="border-color: #aad8d3;">
                <h5 class="fw-bold card-title text-white card-header h4"
                  style="background-color: #aad8d3;border-color: #aad8d3;">GSISd Insights</h5>
                <div class="card-body text-center">
                  <ul class="gateways__list">
                    <li>Global Self-Service</li>
                    <li>GSISD Cloud Dashboards</li>
                    <li>Actionable Insights</li>
                    <li>First, Middle, and Last-Mile Performance Management</li>
                    <li>Application, Cloud, and Flow Visibility</li>
                  </ul>
                </div>
              </div>
            </div>
            <div class="col-md-4 col-lg-3 mb-2">
              <div class="card text-white" style="border-color: #393e46;">
                <h5 class="fw-bold card-title text-white card-header h4"
                  style="background-color: #393e46;border-color: #393e46;">Licensing</h5>
                <div class="card-body text-center">
                  <ul class="gateways__list">
                    <li>Site (small, medium, large)</li>
                    <li>Subscribed bandwidth (SBW + HybridWAN +DIA)</li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!--gateways-->
    <section class="sdwan__features spacer" style="background-color:#e8e9e9">
      <div class="container">
        <h2 class="main-title mb-4 text-center">GSISD Advantages</h2>
        <div class="row mt-5">
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/Integrated_security_with_NGFW_GSISd.png"
                alt="Integrated security with NGFW_GSISD" width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">Integrated security <br>with NGFW</h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/SinglePointOfContact_GSISd.png"
                alt="Single Point Of Contact_GSISd" width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">Support Through <br>Singlepoint-Of-Contact</h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/SLA_GSISd.png" alt="SLA_GSISd" width="100px"
                height="100px">
            </div>
            <h3 class="h5 fw-bold mb-2 text-danger">SLAs Via Converged <br>Backbone From 1st day
            </h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/Multi_Cloud_Direct_Links_GSISd.png"
                alt="Multi-Cloud Direct Links_GSISd" width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">Multi-Cloud Direct<br> Links</h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/Flexibility_GSISd.png" alt="Flexibility"
                width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">Flexibility</h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/Edge_And_Cloud_Security_GSISd.png"
                alt="OPEX Based Cloud Consumption Model GSISD" width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">OPEX Based Cloud<br> Consumption Model
            </h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/Edge_And_Cloud_Security_GSISd.png"
                alt="Edge And Cloud Security_GSISd" width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">Edge And Cloud<br> Security
            </h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/Centralized_Orchestration_GSISd.png"
                alt="Centralized Orchestration_GSISd" width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">Centralized Orchestration<br> With Advanced Analytics
            </h3>
          </div>
          <div class="col-md-4 text-center mb-5">
            <div class="display-5 mb-3"><img src="assets/images/solutions/UpTime_GSISd.png" alt="UpTime_GSISd"
                width="100px" height="100px"></div>
            <h3 class="h5 fw-bold mb-2 text-danger">99.99% Uptime
            </h3>
          </div>
        </div>
      </div>
    </section>
    <section class="sdwan__cta gridtile">
      <div class="container">
        <div class="row">
          <h2 class="text-center mb-4 display-6 text-white">Query Or Concerns?</h2>
          <div class="form-group text-center">
            <a href="contact-us.php" class="btn btn-danger rounded-pill px-4">Contact Us</a>
          </div>
        </div>
      </div>
    </section>
  </main>
  <?php include 'footer.php';?>
  <script src="assets/js/jquery.min.js"></script>
  <script src="assets/js/bootstrap.bundle.min.js"></script>
  <script src="assets/js/main.js"></script>
</body>

</html>